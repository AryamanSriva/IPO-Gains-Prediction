...
class IPOModelTrainer:
    ...
    def prepare_features(self, df, target_col='listing_gains'):
        ...
        feature_categories = {
            ...
            'engineered': ['qib_rii_ratio', 'institutional_ratio', 'price_pe_ratio'],
    ...
    def evaluate_final_model(self, model, X_train, X_test, y_train, y_test):
        ...
        # Scale features if model is linear
        if type(model).__name__ in ['LinearRegression', 'Ridge', 'Lasso', 'ElasticNet']:
            X_train_eval = self.scaler.transform(X_train)
    ...
    def plot_model_performance(self, y_test, y_pred, residuals, model_name):
        ...
        plt.tight_layout()
        plt.savefig('static/plots/model_performance.png', dpi=150, bbox_inches='tight')
        plt.close()
